...
export const resolvers = {
  ...
  getGroupsForGameId: async (parent, { gameId }) => {
    let res;
    console.log(gameId);
    await Group.find({ gameId: gameId }).sort({ creationDate: -1 }).then(data => {
      res = data;
    }, err => {});
    console.log(res);
    return res;
  ...
  getGroupByGroupId: async (parent, { groupId }) => {
    let res;
    console.log(groupId);
    await Group.findOne({ _id: groupId }).then(data => {
      res = data;
    }, err => {});
    console.log(res);
    return res;
  ...
  getFriendsList: async (parent, { userId }) => {
    let res;
    await FriendsList.findOne({ userId: userId }).then(data => {
      res = data;
      console.log(data);
    }, err => {});
    return res;
