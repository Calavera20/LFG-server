...
export const resolvers = {
  signup: async (parent, { username, email, password }) => {
    ...
    let res;
    await new User({ username: username, email: email, password: hash }).save().then(async user => {
      console.log(username);
      res = username;
      await new FriendsList({ userId: user.id }).save().then(() => {}, err => {
        res = err.code;
      });
    }, err => {
      console.log(err);
  ...
  login: async (parent, { username, password }) => {
    ...
    await User.findOne({ username: username }).then(async user => {
      ...
      if (match) {
        ...
        const accessToken = jwt.sign({ id: user.id, email: user.email }, "secret", { expiresIn: '1d' });
  ...
  createGroup: async (parent, { description, creator, playerLimit, gameId }) => {
    ...
    await new Group({ description: description, creator: creator, playerLimit: playerLimit, gameId: gameId, members: [creator], isOpen: true, currentSize: 1, creationDate: creationDate }).save().then(async newGroup => {
  ...
  friendInvite: async (parent, { userData, inviteeData }) => {
    await FriendsList.updateOne({ userId: userData.userId }, { $push: { invited: inviteeData } }).then(async () => {
  ...
  acceptFriendInvite: async (parent, { userData, inviteeData }) => {
    await FriendsList.updateOne({ userId: userData.userId }, { $pull: { pending: { userId: inviteeData.userId } } }).then(async d => {
      await FriendsList.updateOne({ userId: userData.userId }, { $push: { friends: inviteeData } }).then(async d => {
        await FriendsList.updateOne({ userId: inviteeData.userId }, { $pull: { invited: { userId: userData.userId } } }).then(async d => {
